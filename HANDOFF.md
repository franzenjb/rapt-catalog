...
- **Red Cross map-service click fallback** is implemented in `redcross.html` for live map services such as NWS outlooks. If ArcGIS hit testing does not return a feature graphic for a selected map-image service, the app uses the service's REST `identify` endpoint at the clicked map point and opens the same formatted right-side feature panel. Hiding or removing the layer that owns the open detail panel now clears the panel so stale county/Tribal cards do not persist after the layer is off.
